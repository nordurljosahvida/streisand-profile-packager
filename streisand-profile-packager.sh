...
output_ov_co_dir="${output_ov_dir}/combined"
...
do
    ...
    then
        counter_name="$(echo ${counter} | xargs printf %02d)"
        cp ${dir}/*-combined.ovpn "${output_ov_co_dir}/${name_prefix}-ovpn-combined-${counter_name}.ovpn"
        counter=$(($counter + 1))
    fi
done
...
do
    ...
    user_output_ov_co_dir="${user_output_ov_dir}/combined"
    ...
    while [ "${user_profile_counter}" -le "1" ]
    do
        ...
        cp "${output_ov_co_dir}/${name_prefix}-ovpn-combined-${profile_counter_name}.ovpn" "${user_output_ov_co_dir}"
